"""End-to-end test: Shopify Realtime → Stripe Catalog CSV via PFPFactory."""
...
_FIXTURE = _E2E_DIR / "fixtures" / "shopify_realtime_input.json"
_INFRA_PATH = (
    _PYTHON_ROOT / "config" / "shopify_realtime" / "infra_shopify_realtime.yaml"
)
_SHOPIFY_REALTIME_CONFIG_DIR = _PYTHON_ROOT / "config" / "shopify_realtime"
...
def test_shopify_realtime_e2e(monkeypatch) -> None:
    """Shopify Realtime pipeline returns a complete SUCCESS report and valid CSV payload."""
    monkeypatch.chdir(_SHOPIFY_REALTIME_CONFIG_DIR)
    ...
    assert metadata.target == "stripe.shopify_realtime"
    ...
    rows = list(reader)
    # fixture has 4 records: rows 1-3 valid, row 4 missing title → dropped
    ...
    assert row["id"] == "gid://shopify/ProductVariant/201"
    ...
    assert row2["id"] == "gid://shopify/ProductVariant/202"
    ...
    assert row3["id"] == "gid://shopify/ProductVariant/203"
    assert row3["stripe_product_tax_code"] == "txcd_00000000"
    assert row3["availability"] == "out_of_stock"
    assert row3["sale_price"] == "99.99 USD"

    # --- Block 6: drop_invalid — record without title (row 4) is dropped ---
    # fixture[3] has no title field → drop_invalid → only 3 data rows in CSV
    assert (
        len(rows) == 4
    ), f"Expected exactly header + 3 data rows (record 4 dropped), got {len(rows)}"
    ids_in_csv = [dict(zip(header, r))["id"] for r in rows[1:]]
    assert (
        "gid://shopify/ProductVariant/204" not in ids_in_csv
    ), "Record 204 (missing title) should have been dropped"
