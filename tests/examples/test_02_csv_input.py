"""Tests for the 02_csv_input public example."""
...
def _load_example_run_module() -> ModuleType:
    ...
    Returns:
        ModuleType: The loaded `run.py` module for example 02.
    """
    example_dir = Path(__file__).resolve().parents[2] / "examples" / "02_csv_input"
    module_path = example_dir / "run.py"
    spec = importlib.util.spec_from_file_location(
        "example_02_csv_input_run", module_path
    )
...
def test_example_worker_matches_expected_csv() -> None:
    """Verify that example 02 produces the expected CSV artifact bytes."""
    example_dir = Path(run.EXAMPLE_DIR)
    worker = PFPFactory().build_worker(infra_path=example_dir / "infra.yaml")

    report = worker.run((example_dir / "input" / "input.csv").read_bytes())
...
def test_example_runner_prints_success_preview() -> None:
    ...
    assert (
        "SKU-2,CSV Starter,From CSV input,https://example.com/sku-2,in_stock" in output
    )
